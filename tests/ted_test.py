...
class TestTED(unittest.TestCase):
    ...
    def test_outermost_right_leaves(self):
        ...
        np.testing.assert_array_equal(
            np.array([], dtype=int), ted.outermost_right_leaves([])
        )
    ...
    def test_keyroots(self):
        # consider the empty tree
        np.testing.assert_array_equal(
            np.array([], dtype=int), ted.keyroots(np.array([], dtype=int))
        )
        # consider the tree 0(1(2), 3(4, 5))
    ...
    def test_ted(self):
        ...
        for i in range(m):
            for j in range(m):
                D_actual[i, j] = ted.ted(
                    trees[i], adjs[i], trees[j], adjs[j], kron_distance
                )
                self.assertEqual(
                    D_actual[i, j], ted.ted((trees[i], adjs[i]), (trees[j], adjs[j]))
                )
    ...
    def test_ted_backtrace_matrix(self):
        ...
        np.testing.assert_almost_equal(
            np.sum(P[:, : len(z)], axis=0), np.ones(len(z)), 2
        )
        np.testing.assert_almost_equal(
            np.sum(P[: len(y), :], axis=1), np.ones(len(y)), 2
        )
