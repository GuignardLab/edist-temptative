...
class BEDL(BaseEstimator, ClassifierMixin):
    ...
    def __init__(
        self, K, T=5, phi=None, phi_grad=None, distance=None, distance_backtrace=None
    ):
        self.K = K
        self.T = T
        if phi is None:
        ...
        for t in range(self.T):
            ...
            for k in range(len(Ps)):
                for l in range(len(Ps[k])):
                    Ps[k][l] = reduce_backtrace(
                        Ps[k][l][0], X[k], W[l], len(self._embedding)
                    )
            ...
            if res.x is None:
                raise ValueError(
                    "Optimization returned none and failed with message: %s!"
                    % str(res.message)
                )
    ...
    def _loss_and_grad(self, embedding, Ps, y, unique_labels):
        ...
        embedding = np.concatenate(
            [embedding, np.zeros((1, embedding.shape[1]))], axis=0
        )
        ...
        for l in range(len(unique_labels)):
            ...
            closest_plus[inClass_l] = inClass_w_l[
                np.argmin(Dp[inClass_l, :][:, inClass_w_l], axis=1)
            ]
            dp[inClass_l] = Dp[inClass_l, closest_plus[inClass_l]]
            # find the closest prototype in a different class
            closest_minus[inClass_l] = outClass_w_l[
                np.argmin(Dp[inClass_l, :][:, outClass_w_l], axis=1)
            ]
        ...
        for j in range(embedding.shape[0] - 1):
            ...
            for i in range(len(Ps)):
                ...
                Grad[j, :] += (
                    mus_grad[i]
                    * dm[i]
                    * np.dot(P_plus[j, :] + P_plus[:, j], delta_grad_j)
                )
                P_minus = Ps[i][closest_minus[i]]
                Grad[j, :] -= (
                    mus_grad[i]
                    * dp[i]
                    * np.dot(P_minus[j, :] + P_minus[:, j], delta_grad_j)
                )
        # return the results
        if is_flat:
            Grad = Grad.flatten()
    ...
    for X in Xs:
        ...
        if is_tree:
            ...
            if not isinstance(X, list):
                raise ValueError(
                    "Expected either a list or a tree as data point, but got %s"
                    % str(datum)
                )
        # index the datum
        Y = []
