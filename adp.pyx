	...
	for i in range(m-1,-1,-1):
		for r in range(R):
			for (k, s) in adj_del[r]:
				Ds_view[r, i, -1] = Delta_del_view[k, i] + Ds_view[s, i+1, -1]

	# initialize last row for all symbols
	for j in range(n-1,-1,-1):
		for r in range(R):
			for (k, s) in adj_ins[r]:
				Ds_view[r, -1, j] = Delta_ins_view[k, j] + Ds_view[s, -1, j+1]
		...
		for j in range(n-1,-1,-1):
			for r in range(R):
				...
				for (k, s) in adj_rep[r]:
					current_cost = Delta_rep_view[k, i, j] + Ds_view[s, i+1, j+1]
					if(current_cost < min_cost):
						min_cost = current_cost
				# then, consider deletions
				for (k, s) in adj_del[r]:
					current_cost = Delta_del_view[k, i] + Ds_view[s, i+1, j]
					if(current_cost < min_cost):
						min_cost = current_cost
				# finally, consider insertions
				for (k, s) in adj_ins[r]:
					current_cost = Delta_ins_view[k, j] + Ds_view[s, i, j+1]
					if(current_cost < min_cost):
						min_cost = current_cost
